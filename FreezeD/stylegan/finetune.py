...
def backward_G_supervised(args, G_target, real_index, real_image):
	...
	embed = G_target.embedding(real_index)
...
if __name__ == '__main__':
	...
	if args.supervised:
		G_target = StyledGenerator(code_size, dataset_size=len(dataset), embed_dim=code_size).cuda()
		G_running_target = StyledGenerator(code_size, dataset_size=len(dataset), embed_dim=code_size).cuda()
		G_running_target.train(False)
		accumulate(G_running_target, G_target, 0)
	else:
		G_target = StyledGenerator(code_size).cuda()
		D_target = Discriminator(from_rgb_activate=True).cuda()
		G_running_target = StyledGenerator(code_size).cuda()
		G_running_target.train(False)
		accumulate(G_running_target, G_target, 0)

		G_source = StyledGenerator(code_size).cuda()
		D_source = Discriminator(from_rgb_activate=True).cuda()
	...
	if args.miner:
		...
		G_optimizer = optim.Adam(G_target.generator.parameters(), lr=args.lr, betas=(0.0, 0.99))
		G_optimizer.add_param_group({'params': G_target.style.parameters(), 'lr': args.lr * 0.01, 'mult': 0.01})
	...
	if not args.init_G:
		G_target.load_state_dict(ckpt['generator'], strict=False)
		G_running_target.load_state_dict(ckpt['g_running'], strict=False)

	if not args.supervised and not args.init_D:
		D_target.load_state_dict(ckpt['discriminator'])

	if not args.supervised:
		G_source.load_state_dict(ckpt['generator'])
		D_source.load_state_dict(ckpt['discriminator'])
